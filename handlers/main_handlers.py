...
async def start_command(message: types.Message):
    await message.answer("Привет, выбери нужную команду из предложенных", reply_markup=kboard)
...
async def load_habit_details(message: types.Message, state: FSMContext):
    ...
    await message.answer("Новая привычка добавлена")
    await state.finish()
...
async def delete_habit(message: types.Message):
    ...
    await bot.send_message(message.from_user.id,'Выбери привычку, которую хочешь удалить:')
    for ret in read:
...
async def delete_plan(message: types.Message):
    ...
    await bot.send_message(message.from_user.id,'Выбери план, который хочешь удалить:')
    for ret in read:
...
async def check_habit(message: types.Message):
    ...
    await bot.send_message(message.from_user.id,'Выбери то, что ты выполнил сегодня:')
    for ret in read:
        await bot.send_message(message.from_user.id, text=f'{ret[1]}\nДетали: {ret[2]}', reply_markup=InlineKeyboardMarkup().\
